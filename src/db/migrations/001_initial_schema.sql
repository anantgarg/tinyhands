...
CREATE TABLE IF NOT EXISTS agents (
  ...
  respond_to_all_messages BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE OR REPLACE FUNCTION source_chunks_search_trigger() RETURNS trigger AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_source_chunks_search ON source_chunks;
CREATE TRIGGER trg_source_chunks_search
  BEFORE INSERT OR UPDATE ON source_chunks
  FOR EACH ROW EXECUTE FUNCTION source_chunks_search_trigger();
...
DROP TRIGGER IF EXISTS trg_agent_memory_search ON agent_memory;
CREATE TRIGGER trg_agent_memory_search
...
DROP TRIGGER IF EXISTS trg_kb_chunks_search ON kb_chunks;
...
-- (Must be created before code_artifacts due to FK dependency)
