...
// ── Manual Wizard (original 4-step form) ──
...
// ── Main component: chat mode (default) or manual wizard (fallback) ──
...
export function AgentCreate() {
  ...
  // When creationMode is turned off (X button clicked) and we're not in manual mode yet,
  // switch to manual mode
  useEffect(() => {
    if (!creationMode && !manualMode) {
      // Only switch if we've been mounted for a bit (not the initial render before enterCreationMode fires)
      const timer = setTimeout(() => {
        if (!useChatStore.getState().creationMode) {
          setManualMode(true);
        }
      }, 100);
    ...
    <div>
      ...
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 text-sm text-warm-text-secondary hover:text-warm-text mt-2 transition-colors"
        >
          Or set up manually
