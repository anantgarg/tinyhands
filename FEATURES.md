...
- Periodic health check runs every 30 minutes in the sync process, iterating every active workspace so multi-tenant installs get their connections checked (not just the env-token workspace).
...
### KB Source Sync (Every 5 Minutes via Sync Process)

- Sync process (`src/sync.ts`) runs a cross-workspace KB auto-sync tick every 5 minutes.
- A source is re-synced when `auto_sync=true` and `NOW() - last_sync_at >= sync_interval_hours`.
- On each tick (and on sync-process startup), rows stuck in `status='syncing'` for more than 30 minutes are reset to `status='error'` so they become eligible again after a crash or restart. This prevents sources from silently hanging after the listener is force-restarted by the watchdog mid-sync.
- Manual "sync now" triggers (Slack command, dashboard API, flush-and-resync) still run in the listener process for user-initiated syncs.
- Each source connector (`src/modules/kb-sources/sync-handlers.ts`) handles fetching and indexing from its data source and updates `last_sync_at` + `entry_count` on success, or `status='error'` + `error_message` on failure.
...
- **Alerts**: posted to `#tinyhands` Slack channel. Built-in rules: high error rate (>10%), expensive runs (>$5), daily budget exceeded, queue depth warnings, long-running jobs. Alert checks iterate every active workspace each minute and post with that workspace's own bot token.
- **Daily digest**: automated summary posted to Slack -- run count, token usage, cost, top agent, top user, high error agents, anomalous cost spikes (>2x 7-day average). Generated once per active workspace per day by the sync process, using the workspace's bot token.
